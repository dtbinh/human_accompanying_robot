% 7/15/17
% This code now contains ROS publisher and subscribers

function outPara = pathPlannerROS(agent,inPara)
...
%%% use ROS path planner (controller.jl)
%
% send the data to ROS path controller
humanPred_pub = rospublisher('/humanPred','std_msgs/Float64MultiArray');

% define message object
hPred_msg = rosmessage(humanPred_pub);

% a complicated story. layout property of Float64MultiArray is of class
% MultiArrayLayout, so I need define multilayout first. But the Dim
% property of multilayout is of class MultiArrayDimension, so I need to
% define multidim first.

hPred = [x_h;repmat(h_v,1,size(x_h,2))]; %[x;y;vx;vy]

multidim(1) = rosmessage('std_msgs/MultiArrayDimension');
multidim(1).Label = 'state';
multidim(1).Size = size(hPred,1);
multidim(1).Stride = numel(hPred);
multidim(2) = rosmessage('std_msgs/MultiArrayDimension');
multidim(2).Label = 'horizon';
multidim(2).Size = size(hPred,2);
multidim(2).Stride = size(hPred,2);

multilayout = rosmessage('std_msgs/MultiArrayLayout');
multilayout.Dim = multidim;

hPred_msg.Layout = multilayout;
hPred_msg.Data = hPred(:); % Data is a 1-D vector

% send message
send(humanPred_pub,hPred_msg)

% receive control input
sub_w = rossubscriber('/mpc_out_steer','std_msgs/Float64MultiArray');
sub_a = rossubscriber('/mpc_out_acc','std_msgs/Float64MultiArray');
msg_w = receive(sub_w,10)
msg_a = receive(sub_a,10)
sub_x = rossubscriber('/robot_state','std_msgs/Float64MultiArray');
opt_u = [msg_w.Data';msg_a.Data'];
msg_x = receive(sub_x,10)
opt_x = reshape(msg_x.Data,4,length(msg_x.Data)/4)
display('messages received')
%}

%%% use original controller coded in MATLAB
%{
% define parameters
