...
% 7/13/2017
% the file is modified for ROS-in-the-loop simulation for debugging ROS
% controller. 
...
rosshutdown % close rosinit if there exists any
% rosinit('192.168.0.160') 
% rosinit('192.168.0.160')
setenv('ROS_MASTER_URI','http://192.168.0.160:11311') % use ifconfig to find the ip address (wlan0) of Ubuntu machine and put it here.
setenv('ROS_IP','192.168.0.114') % use this laptop's own ip. ipconfig, wireless LAN adapter Wi-Fi, IPv4 address
rosinit
% node = robotics.ros.Node('/testTime');
% rate = robotics.ros.Rate(node,5);
rate = rosrate(5);

%% Setup
% include solvers in YALMIP
...
plan_state = zeros(4,hor+1,kf); % robot's current and planned future state [x,y,theta,v]
...
reset(rate)
for k = 1:kf
    rate.TotalElapsedTime % output the time of each iterartion
    display(k)
...
%         agents(1).currentV = h_v(wp_cnt);
    ...
    [outPara_ams] = agentMoveROS(inPara_ams);
