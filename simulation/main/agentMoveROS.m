...
% 7/15/17
% This code now contains ROS publisher

function [outPara] = agentMoveROS(inPara)
%% initialization
% get input arguments
...
%         pos_pre_imm = inPara.pos_pre_imm;
        ...
        if strcmp(plan_type,'MPC')         
            ...
            %}
            outPara_pp = pathPlannerROS(agent,inPara_pp);
            ...
            %%% send robot state to controller.jl on ROS   
            %
            robot_pub = rospublisher('/robotState','std_msgs/Float64MultiArray');
            
            % define message object
            r_state_msg = rosmessage(robot_pub);
            r_cur_state = opt_x(:,2);
            r_state_msg.Data = r_cur_state;
            send(robot_pub,r_state_msg)
            display('messages sent')
            
            % test the publisher
            %%% very very strange. The Ubuntu cannot receive /robotState 
            %%% unless the test file is run, i.e., a subscriber is
            %%% listening to /robotState. Don't know why but at least this
            %%% can make it work. So I keep it here.
            test 
            %
