function problem = rrt_star(fld, max_iter, is_benchmark, rand_seed, variant)
...
for k = 1:problem.obstacle.num
    p2 = fill(problem.obstacle.output{k}(1:end, 1), problem.obstacle.output{k}(1:end, 2), 'r');
    set(p2,'HandleVisibility','off','EdgeAlpha',0);
    drawnow
%     problem.plot_circle(problem.obstacle.cir_center{k}(1),problem.obstacle.cir_center{k}(2), problem.obstacle.r(k));
%     set(p2,'HandleVisibility','off','EdgeAlpha',0);
end
